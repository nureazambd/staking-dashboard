...
function BlockHeader_Scanner ({ value }: Props): React.ReactElement<Props> | null {
  if (!value) {
    return null;
  }
  ...
  return (
    <tr>
      ...
      <td className='all hash overflow'>{value.author && (
          <AddressSmall value={value.author} />
        )}</td>
      {/* <td className='address'>
        {value.author && (
...
export default React.memo(BlockHeader_Scanner);
